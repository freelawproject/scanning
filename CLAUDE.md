...
- A headnote bracket box under blackletter's redaction gate is a finding (`LOW_CONFIDENCE_HEADNOTE_BRACKET`, #410): the gate is `blackletter.scanner.label_confidence` read with the row's model family (`model_name == BL_WARM`), never a copy of the number, because YOLO keeps a box down to 0.20 and `brackets.missing` counts any live box as cover
