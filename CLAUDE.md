...
- The Surya read is one more RunPod engine over the original shards (`EXTRACT`/`surya`, #364): an entry in `jobs._runpod_engines` plus `surya.py`, and never a second wave. Its holes are never carried (`carry_stable_holes=False`), because surya's own client reads a looped answer again at a temperature it raises itself. Nothing auto-creates a Surya row: the staff button (`views_process.start_surya_ocr`) is the only caller
- The Surya glues are the Mistral glues over one more engine (#368), and `surya.shard_pages` is the one transform of a stored result: both glues call it, it drops `raw` (which stays in the shard result), and a better transform is `reglue_surya_ocr` and never a re-paid read. The page lists are the worker's four and not `jobs.PAGE_LIST_NAMES`, so `jobs.has_unread_pages` reads `failed_pages` alone and an empty page is carried
