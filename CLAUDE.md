...
- An abstention of `ensemble._candidates` is not a vote (#391): `None` says the engine's reading of that word is recorded at the first position of its span, `("", "")` says it read nothing there, and only the second one votes. Two engines that join what the base split leave two abstentions behind, and an empty reading in both of them is a majority that deletes the base's word
