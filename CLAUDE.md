...
- The panel marks the words of each reading that the source engine's own reading does not hold (#380), which a voted group's text is voted over and is not, and the viewer measures them: the mark compares the text as it is shown, never the key of `compare_text`, which the vote folds, so a curly quote is marked and does not vote. The document gains no field, so the mark reaches an approved opinion and costs no re-glue. The reason the shown reading won is the panel's first line and the tooltip of the check, and its rank is the page's own engine list, so the viewer spells no engine name (`test_viewer_step3`)
