...
## Review 3 (#335, models only)

- An `Opinion` is keyed by `(scan, first printed page, index in that printed page)`, stamped at creation. Not the boundary anchors: a computed `OpinionBoundary` is rebuilt at every compute and a new detection model moves a box past `ANCHOR_TOLERANCE_PT`. A later reading change is a card, never a new key. `page_count` holds the physical count, so no check does arithmetic on the printed numbers
- `OpinionText` is one row per **page** of an opinion, and it holds an engine reading only where the engines disagree. The whole read of every engine is already on S3 (the Mistral `result_key`, the glued dots.mocr run), so a better comparison rule is a re-glue at no API cost. `text` is a cache; `human_text` is the truth and nothing discards it. An edit moves the offsets of `disagreements`, so it answers that page
...
- A warning of review 3 is an `OpinionFinding`, not an `Issue`: a third family in that table would make every reader a two-set test. It keeps the review-2 rules: one rebuild writes them, a dismissal is its own row that nothing deletes, and a `STALE_OPINION_CHECKS` row cannot be dismissed
