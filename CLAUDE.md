...
- `vote_words` says what the vote did with every word of a voted group (#380): `majority` where a majority settled it short of every engine, `low_confidence` where nothing settled it, and no flag where every engine read it alike. The browser never derives those three from the readings, or a folded quote would read as a vote
