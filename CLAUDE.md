...
- The job creators are pinned by an AST test (`TestKnownEnqueuePaths`): the pipeline, `start_dots_mocr`, `start_mistral_ocr`, the two sweeps (`dots_mocr.enqueue_missing_runs`, `yolo.enqueue_missing_runs`), `enqueue_yolo_detect`, `apply.py`, `reread_failed_pages`. Row creation is what costs money
...
- DONE and every later status lock the ten page-edit endpoints (`_refuse_locked_edits`, `LOCKED_STATUSES`, 409 with `EDITS_LOCKED_MESSAGE`); `dismiss_issue` is not locked. The viewer follows `page_edits_locked`. The reopen is a staff button: DONE to READY by compare-and-swap, and it supersedes the run
- `PageRepairRequest` (#249) is not a `PageEdit`. Fulfilled is derived (`annotate_fulfilled`: a later, standing insert or replace at the same address under the same fingerprint, or a replacement of either page beside the gap of a one-page INSERT request when no other open request claims that page, #393), never stamped. The neighbour rule fires on a clear-cut case alone; an ambiguous one waits for a person to dismiss. Dismissed, never deleted, by any logged-in user, from the page card or the Repairs list. A stale request still waits (#266), unlike a stale `PageEdit`. `repairs.py` owns the one definition of "waiting"
- Every open INSERT request has a placeholder in step 1 whether or not the sequence shows its gap (`repairs.project_requests`, #393): a projection for the viewer, never a `missing` entry written into `page_map`, which the sequence checks read
...
- The other engines fill the pages dots.mocr left blank (#351): `ocr_results_from_volume` takes their glued volume documents (`page_numbers.fallback_documents`, the same call in `run_compute_issues` and in the apply's `printed_pages`), walks each with `engine_candidates` through `opinion_ocr.ENGINES` (whose `band_labels` and `zone_prefix` are the engine's part; Mistral's `footer` is footnote text and is no band label), and dots.mocr's own reading is the pick wherever it has one, overruled only when both neighbours ask for another engine's number. Every line is folded by `ensemble.compare_text` first, the fold of `carries_number`. A word of a longer line is a number at the corner alone, or the volume number of the running head is read as the page. The Mistral and Surya volume glues call `dots_mocr.reopen_apply_after_read`, so the numbers are read again on the next tick; a fallback that does not load is logged and left out, never a hold on review 1
...
- A move (#261) is the card's answer to a transposed pair: the view offers the swap on a `backward_page` card only when the two PDF pages are adjacent and the printed numbers step back by one, and never parses the message. `page_edits.slot_order` is the one rule of the corrected order, read by `plan_run` and by `order_by_moves`, which feeds the sequence analysis and the sidebar; `Scan.ocr_results` and `page_map` entries keep their `pdf_page` addresses, only their order follows
- `_project_trailing_gap` (#256) puts one range placeholder on a collapsed missing run at the end of the volume only, from both `page_map` builders, after the last copy of the greatest number printed below the run: the rule blackletter gives a short gap past the last number (#403)
- An unnumbered page's `logical_number` is its PDF page, a display value (#403): no placement and no card link reads it as a printed number. A card resolves a printed number through the pages that print it (`views_process._prints`), and a `missing_page` card links by the `anchor_pdf_page` of its placeholder or of the upload that filled it
...
- An approval of a model detection is a move by zero (#414): `detections.approve_model_row` calls `move_model_row` at the row's own box, so the model row gets a dismiss and the curator a hand-drawn row that every import keeps. `DetectionDecision.Kind.APPROVE` has no writer; `resolve` and `_apply_effect` keep landing the rows written before. The endpoint (and the add of a box over a model box) answers `detection_id` and `replaced_id`, and `adoptDetection` in `viewer_step2.js` is the one place the browser follows them
...
- The margin measure reads a copy of each page whose `TEXT_COLUMN` (x only) and `IMAGE` (both axes) boxes are held inside the text box plus `DEFAULT_BUFFER` (`margin_fit.clipped_pages`, #370), because a box the model draws onto a blot pins the strip at its edge (`_shrink_rects_for_detections`); the clip keeps a box that would lose over half of itself and skips a page narrower than `MIN_TEXT_WIDTH_FRACTION`, the two refusals visible from outside, and the document's own boxes never change. Every page gets four strips (`margin_fit.ensure_strips`, `MIN_STRIP_PT`): a missing one is the curator's handle at the page edge, held off the detections by blackletter's own pull-back and withheld when that collapses it, because a strip never covers a detection
- A computed text redaction is written inside the content box of its page's strips (`redactions.strip_content_box`, over `margin_fit._sides`, #371): all four sides, smaller only, never dropped. blackletter's `footer_top` is the page height on a page with no bottom-margin detection and its ink clamp follows a blot to the page edge, so the strips are the one bottom bound, and the handle a page gets where the measure gave none is pushed off every `TEXT_COLUMN`, so a text rect loses the blot to it and never its text
...
- A headnote bracket box under blackletter's redaction gate is a finding (`LOW_CONFIDENCE_HEADNOTE_BRACKET`, #410): the gate is `blackletter.scanner.label_confidence` read with the family the compute reads (`rows_are_bl_warm` over the live model rows' `found_by`), never a copy of the number, because YOLO keeps a box down to 0.20 and `brackets.missing` counts any live box as cover. The card offers the approval, which lifts the row to 1.0
...
- Every write of review 2 refuses while the volume is in review 1 (`views_api._refuse_closed_review`, 409 over `review_states.PREVIEW_STATUSES`, #388): a row written there is built into nothing, and the first compute under the new run imports the model rows again and measures the human ones against a space nobody approved. The legacy `PENDING_REVIEW` step 2 and the two #263 statuses keep their own rules. `test_detection_preview` pins the gate against `test_success_messages.WRITE_VIEWS`
- The detection preview (#388) is `review_states.preview_only`: a merged volume detection run, a status in `PREVIEW_STATUSES`, no apply in progress and review 2 not ready. It draws the merged document (`yolo.preview_entries`) over the bitonal copy and computes nothing; `serve_detections` answers it in place of the rows, and the redactions and the boundaries answer empty, so no row of a superseded run reaches the page. A render turns the rule into `preview_only`, which is step 2 alone: step 1 of the same volume is an open page review and keeps its edits
...
- A unit's `text` is plain and its formatting is `marks` beside it (#404): `EngineSpec.dialect` over `markup_key` (Surya's is `html`, whose flattened `text` lost the superscripts) with the label's `kind_types` is the one parse of an engine's dialect into `markup.Parsed` (`em`/`strong`/`sup` as offsets, `kind` in `markup.BLOCK_KINDS`, `table` rows), run in `build_document` before the bracket strip, whose spans move the marks (`markup.shift`). The volume documents stay as the engines wrote them, so a better parse is `reglue_opinion_ocr` and never a corpus re-glue. `Parsed.text` keeps one space between words and a `\n` beside no other whitespace, or `ensemble.plain` moves every offset
...
- The pass is eleven of the collect tick and takes a row with `ocr_engine_count` at least `OPINION_ENSEMBLE_MIN_ENGINES` (3), stamped by the OCR glue (#365): a vote of two engines settles nothing. Surya (#368) is the third engine, so a volume whose run carries all three keys opens the pass by itself; the `rerun_opinion_ensemble` command waives the gate for a volume read by two, the endpoint waives it too, and the viewer PR puts its button on the page. A fault of the row spends `ensemble_attempts` and ends at ERROR; a `TransientFault` (the bucket) spends nothing, and a missing object is a fact about the row. `opinions.create_rows` resets the counter
- The alignment is geometry alone, in the points of the volume page (#317/#365): a unit that reads nothing does not link, whatever its size, and is attached to the group it covers most as a `silent` engine. A group any of whose units carries an exclusion is dropped after the alignment, never before it, and the group goes whole, so a reading in it that no exclusion covers is what `PARTIAL_REDACTION` counts
- `ensemble.reading_order` is the one rule for the order of a page's boxes, and both levels call it (#365): the groups of a page, and the members of one engine inside a group. A group can span the gutter, and members ordered by line alone read across it. The order reads no `Detection` row, so a curator's edit never moves the text
- The vote compares `ensemble.compare_text` and shows the winner's own text (#365); an engine that read nothing does not vote. No document stores markup (#404): a group's `text` is plain, its formatting is `marks` beside it (standoff `em`/`strong`/`sup`, offsets into that text) and `kind` on the group, a voted group stores tokens with a `low_confidence` flag, `markup.serialize` is the one writer of a tagged string, and the viewer builds the nodes. `ensemble._differs` is the one rule for "the engines did not read this group alike" — a majority, a word vote, a silent engine, or fewer engines in the group than the document holds — and the `ENGINES_DISAGREE` card and the `OpinionText.disagreements` entry both read it
- The marks of a group are a word-level union over the engines that read it, aligned by the word keys of `compare_word` and never by position, and its `kind` is a majority of those engines with the rank breaking a tie (`ensemble.union_marks`, `ensemble.group_kind`, #404). The engines are silent, not wrong (dots marks three italics where Mistral marks one), so a vote on a mark would delete it; a union of the kinds would make every caption Mistral labels `title` a heading. `OpinionText.marks` is a cache of `text` and `footnotes` like `disagreements`, in the offsets of the field its `section` names
- A page with no text at all — no engine read it, or no engine measured it — carries an `error` and is one `PAGE_NOT_READ` card and no other (#365). A drop names what covered it, because `opinion_ocr.verdict` writes the mask of the opinion before as well as a redaction
- The ensemble is the one writer of the `OpinionText` rows and of `ENSEMBLE_CHECKS`, and `opinions.create_rows` keeps the two stale checks (#365). `text` and `disagreements` are a cache; nothing there reads or writes `human_text`. An approved opinion is never written over: `due()`, the button and the command refuse `TEXT_REVIEW_DONE`. A standing `OpinionFindingDismissal` of the same page and check mutes the new card
- `opinions.promote_ready` and `opinions.demote_stale` are the two writers of `READY_FOR_TEXT_REVIEW`, and `opinions.text_review_ready` the one rule behind both: the redacted PDF and the text of the live revision, read off the row (#365). The PDF pass and the ensemble pass know nothing of each other and either can be last, so the state is pass fourteen of the collect tick (`promote_ready_opinions`), the twin of `review_states.promote_ready_scans`; it walks both directions under a cap, because a re-glue raises the revision and leaves the status where it was. The promotion reads `opinion_pdf.OPINION_PDF_STATUSES` and the demotion reads no scan status
- A failure of the work that prepares an opinion ends every row but an approved one (#365): `opinion_pdf._fail` and `ensemble.record_failure` both exclude `TEXT_REVIEW_DONE` alone. A row at the cap leaves `opinion_pdf.owed`, so a status the swap cannot name is a dead row nobody is told about
- The OCR glue deletes the bracket token at the start of every line of a unit that a bracket box touches (`opinion_ocr.is_bracket_box`, `brackets.strip_line_tokens`, #373), and a `HEADNOTE_BRACKET` box does not go into `verdict`: the box is a glyph in a paragraph, so a share over it leaks the token or drops the paragraph. A bracket box is a `HEADNOTE_BRACKET` redaction (the model's, or a curator's detection after a recompute), or a `manual` redaction with the size and the place of a bracket (`MANUAL_BRACKET_MAX_*`), because a curator draws that type over anything
- A printed page number is the third verdict of the OCR glue (`opinion_ocr.PAGE_NUMBER`, #396), after the redaction and the neighbour mask: the unit is in the head or the foot zone, by its band (`page_numbers.band_of`) or by the engine's label (`page_numbers.is_head_or_foot_label`), and one of its lines ends in the approved number of its page (`page_numbers.carries_number` over the run's printed-page map, folded by `ensemble.compare_text` first), judged in each engine's own text and never off a dots.mocr box. Both are required, or a body line that ends in the same digits goes with it; a page whose approved number the engines did not write (a corrected misread, no number) keeps every unit
- The review page of an opinion reads the three ledgers (`opinion_pdf.is_written`, `opinion_ocr.is_written`, `ensemble.is_written`) and never the bucket, and `opinion_file_index` is its `files` index (#334/#365). The OCR stamp is one over every engine document of the revision, so an engine document is written only when the run also carries that engine's key, and an entry with no `url` is an object nothing wrote
- The browser reads the two objects of the review page from a presigned GET that `opinion_pdf_url` and `opinion_ensemble_url` answer in JSON (#365), because pdf.js and `fetch` read a direct URL and a browser judges a redirect's CORS rules differently. `serve_opinion_pdf` stays the download and the tab (`?disposition=inline`), the one route that answers `SAMEORIGIN` where the site answers `DENY`; `serve_opinion_ocr` takes `ensemble` beside `manifest` (`views_process.EXTRA_OPINION_OBJECTS`), and the ledger of that name is the ensemble's own
- A group the engines did not read alike carries a badge that opens one reading per engine (#365), and `differs` in `viewer_step3.js` is the browser's copy of `ensemble._differs`, the rule the `ENGINES_DISAGREE` card and `OpinionText.disagreements` already read. The panel is built with `textContent`, like every other node of that viewer
- The panel marks the words of each reading that the text above does not hold (#380), and the viewer measures them: every line is compared with `group.text` and never with the line that carries the check, because a voted group's text is voted over that line and leaves it wherever the others outvoted it. The mark is over the text as it is shown, never over the key of `compare_text`, which the vote folds, so a curly quote is marked and does not vote. The reason the shown reading won is the panel's first line, the tooltip of the check and the button that opens the rules of the vote, and the rank is the page's own engine list, so the viewer spells no engine name (`test_viewer_step3`)
- A carried reading of `ensemble._candidates` votes what its head won (#391): `Carried` says the engine's reading of that word is recorded at the first position of its span, `("", "")` says it read nothing there, and the two are never one value. The carried reading votes to drop the base's word when the head took its own span, which holds that word already, and votes not at all when the head went another way. Read one of the two as the other and a span the engines joined writes its tail twice, or a word two engines only joined is deleted
- `vote_words` says what the vote did with every word of a voted group (#380): `majority` where a majority settled it short of every engine, `low_confidence` where no majority settled it or the base engine never read it (`inserted` tells those two apart, and they read differently), and no flag where every engine read it alike. The browser never derives them from the readings, or a folded quote would read as a vote
- `viewer_step3.js` draws every box from `box_pt` at every render and builds every node with `textContent` (#365): the document holds tokens and no markup, `groupNode` picks the element by the group's `kind` (`KIND_ELEMENTS`) and `markedNodes` cuts the text at the mark edges into nodes (#404), and a box takes no pointer, so one hit test on the wrapper opens the box under it. The page fits the width of the column until a zoom button moves it; a page holds the scale it was drawn at, so it is drawn again only when that scale changes, and it gives its canvas back when it leaves the column. A marked box goes to the middle of the column, both ways, because a zoomed page is wider than it. Each column is its own scroll box, so a jump moves one column and never the window, and a jump to a page the observer has not drawn takes the container of the page
- The footnote zone of a page is the `FOOTNOTES` detections of the run, written by the OCR glue as `zones.footnotes` on every engine's page (#399): the ensemble reads no `Detection` row, and a moved box is a re-glue. `ensemble.section` is the one rule for which section a group takes, over the aligned group and never a unit, and the zone alone decides: the engines' footnote labels (`EngineSpec.footnote_types`) are exact and rare, so they never move a group, and a labelled group outside every zone is the `FOOTNOTE_UNSURE` card. The viewer reads the section off the group (`sectionOf` in `viewer_step3.js`) and never off its place in the list, and a group with no section is body text
- `OpinionText.footnotes` is a cache like `text`, and a `disagreements` entry names its `section`, the field its offsets point into (#399)
...
- The text overlay draws one engine at a time (#381): `opinion_ocr.ENGINES` is the one table of the engines and of their document fields, `JobEngine`'s own choice label is the one name a person reads (`engine_label`), `scan_ocr_text_url` answers the fields beside the URL and `ocr_text_engines` says which engine read the space the viewer draws, so `ocr_text.js` holds no engine name and a fourth engine is one entry
