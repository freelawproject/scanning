...
- The detection preview (#388) is `review_states.preview_only`: a merged volume detection run, a review-1 status that is not busy, no apply in progress and review 2 not ready. It draws the merged document (`yolo.preview_entries`) over the bitonal copy and computes nothing; `serve_detections` answers it in place of the rows, the redactions and the boundaries answer empty, and every write of step 2 refuses with 409 (`views_api._refuse_preview`). A render turns the rule into `preview_only`, which is step 2 alone: step 1 of the same volume is an open page review and keeps its edits
