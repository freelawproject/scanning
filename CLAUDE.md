...
- A move (#261, #395) is the card's answer to a span scanned out of its order: `page_edits.sorted_window` is the one rule for which `backward_page` card offers a button (the smallest window whose numbers are a shuffle of one consecutive run that both neighbours continue; a step whose window never closes is a misread), `page_edits.rows_for_order` derives the rows from the whole corrected order with that window sorted in place, and the list form of `move_page` replaces the standing set with them. The view keys the offer off the OCR entries and never the message, judges each span once, and a printed number the volume holds twice gets no button. Several moves landing on one anchor go in `ordinal` order, which `moves_by_page` yields and `slot_order` trusts: a reversed span has no anchors that put it right by page order alone
