...
- `/opinions/` lists the `Opinion` rows and `/opinions/legacy/` the frozen `OpinionScan` rows (#334). The step-3 tab links the first with the scan as its filter, and only when the rows exist; a legacy volume keeps `?step=3`. The flag is `review3_opinions`, because `opinion_count` already names the boundaries of step 2
