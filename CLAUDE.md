...
- `viewer_step3.js` draws every box from `box_pt` at every render and builds every node with `textContent` (#365): the document holds tokens and no markup, and a box takes no pointer, so one hit test on the wrapper opens the box under it. The scale fits the column, the width it has and the height it shows (its `max-height`, which the stylesheet owns), and never above 1.2; a page holds the scale it was drawn at, so it is drawn again only when that scale changes, and it gives its canvas back when it leaves the column. Each column is its own scroll box, so a jump moves one column and never the window, and a jump to a page the observer has not drawn takes the container of the page
