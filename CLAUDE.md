...
- `viewer_step3.js` draws every box from `box_pt` at every render and builds every node with `textContent` (#365): the document holds tokens and no markup, and a box takes no pointer, so one hit test on the wrapper opens the box under it. Each column is its own scroll box, so a jump moves one column and never the window
