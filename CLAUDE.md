...
- The alignment is geometry alone, in the points of the volume page, and a group any of whose units carries an exclusion is dropped after it, never before (#317/#365). A unit that reads nothing does not link, whatever its size, and is attached to the group it covers most as a `silent` engine: a picture box would chain the page and a big text block **is** the text it covers. The reading order comes from the boxes (three bands, a column boundary off the left edges) and reads no `Detection` row, so a curator's edit never moves the text
