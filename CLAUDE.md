...
- `OpinionText.footnotes` is a cache like `text`, and a `disagreements` entry names its `section`, the field its offsets point into (#399). The tagger reads `text` alone
- The footnotes are ordered apart from the body by the same `reading_order` and the same column boundary (#399): `build_page` reads the boundary over every group of the page and passes it to `place` twice, so the body joins across a footnote at the foot of a column
