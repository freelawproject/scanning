...
- The text overlay draws one engine at a time (#381): `opinion_ocr.ENGINES` is the one table of the engines, of their labels and of their document fields, `scan_ocr_text_url` answers those fields beside the URL and `ocr_text_engines` says which engine read the space the viewer draws, so `ocr_text.js` holds no engine name and a fourth engine is one entry
