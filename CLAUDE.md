...
- The file is stored before the row and removed if the row loses (`_save_page_file_row`). The first bytes decide the kind (`_accept_page_upload`). Images live on the default storage under `page_edits/`
