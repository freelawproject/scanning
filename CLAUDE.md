...
- A printed page number is the third verdict of the OCR glue (`opinion_ocr.PAGE_NUMBER`, #396), after the redaction and the neighbour mask: the unit sits in the head or the foot band (`page_numbers.band_of`) and one of its lines ends in the approved number of its page (`page_numbers.carries_number` over the run's printed-page map), judged in each engine's own text and never off a dots.mocr box. Both are required, or a body line that ends in the same digits goes with it; a page with no approved number keeps every unit
