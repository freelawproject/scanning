...
- A computed text redaction is written inside the content box of its page's strips (`redactions.strip_content_box`, #371): all four sides, smaller only, never dropped, and a side with no strip is the page edge. blackletter's `footer_top` is the page height on a page with no bottom-margin detection and its ink clamp follows a blot to the page edge, so the strips are the one bottom bound
