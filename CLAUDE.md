...
- A move (#261, #395) is the card's answer to a span scanned out of its order: `page_edits.sorted_window` is the one rule for which `backward_page` card offers a button and which rows it writes (the smallest window whose numbers are a shuffle of one consecutive run that both neighbours continue; a transposed pair, a displaced page, two blocks the wrong way round and a reversed span are all of it; a step whose window never closes is a misread), the view keys it off the OCR entries and never the message, and a printed number the volume holds twice gets no button. Several moves landing on one anchor go in `ordinal` order, which `moves_by_page` yields and `slot_order` trusts: a reversed span has no anchors that put it right by page order alone
