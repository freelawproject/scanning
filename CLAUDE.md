...
- `opinions.promote_ready` is the one writer of `READY_FOR_TEXT_REVIEW`, and `opinions.text_review_ready` the one rule behind it: the redacted PDF and the text of the live revision, read off the row (#365). The PDF pass and the ensemble pass know nothing of each other and either can be last, so the promotion is pass fourteen of the collect tick (`promote_ready_opinions`), the twin of `review_states.promote_ready_scans`
