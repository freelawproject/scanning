...
- The redacted PDF of an opinion (#336) is a glue at `opinion_pdf.key`, under `Opinion.glue_prefix`, cut by `blackletter.api.generate` with `full_redacted=False` over a source that holds the opinion's pages alone, never the volume: the payload comes from the rows in that source's space (`opinion_pdf.payload`), the dict carries `filename` and no printed page so the name is internal, the neighbours' text is masked by `boundaries.outside_rects` with the document (so the masks grow to the ink, which the viewer's masks do not), and the pictures come through `image_for` (`opinion_pdf.image_source`): JPEG at 150 dpi from the shard the page map names, the rect scaled to the shard page, `None` and never a raise for a picture that cannot be read. `redacted_pdf_revision == glue_revision` is the one rule for "the PDF exists" (`opinion_pdf.is_written`), and `opinion_pdf.due` for "a PDF is owed"
- The PDF pass is a fact on the row and no hand-off: one PDF per tick (`opinion_pdf.PDFS_PER_TICK`), newest scan first, no `QueuedAction` and no status write on the scan. A fault, including a non-empty `result["failed"]`, counts on `pdf_attempts`; at `opinion_pdf.MAX_ATTEMPTS` a `PROCESSING` row is `ERROR` with `error_message`, loud then quiet, and the next approval brings it back
- The pass never deletes an input inside a tick: the bitonal mirror and the shards stay until the scan has no due row, or until the daemon starts (`opinion_pdf.release_mirrors`). Its outputs go in the `finally` of the tick that made them. The printed range is the download name alone (`serve_opinion_pdf`, #165)
