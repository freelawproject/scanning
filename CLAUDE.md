...
- `opinion_ocr.write` is the one writer of an opinion's OCR documents (#350): one `{engine}.json` per engine of `opinion_ocr.ENGINES` plus a `manifest.json` written last. Every unit of the opinion's pages is in the file with its verdict (`exclusion`, `share`, in the three bands of `EXCLUDE_SHARE` and `FULL_SHARE`); nothing is dropped before the ensemble aligns, and `kept_units` is the one reader that applies the verdict. The page `md` is never copied
- `ocr_glue_revision == glue_revision` is the one rule for "the OCR glue exists" (`opinion_ocr.is_written`, #350). The glue is pass ten of the collect tick, `OPINIONS_PER_TICK` rows of the newest scan; a fact about the scan (no final run, a stale redaction set, an engine the run owes per `engines_owed`) holds the scan and spends no attempt; a fact about the row spends `ocr_glue_attempts`, then ERROR. A late engine is a re-glue (`reglue_opinion_ocr`), never a watcher
