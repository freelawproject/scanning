...
stale-scan recovery, the legacy-action park, and the uncapped intake
(issue #218)."""
...
class TestUncappedIntake(TestCase):
    """The tick admits a scan whatever the external queues hold.

    Issue #218 capped intake at DAEMON_MAX_ACTIVE_SCANS because a job
    row's queue ceiling started at creation, so an admitted backlog
    expired unsubmitted and sank its volumes to ERROR. The ceiling now
    starts at the attempt's first claim and a row in our own queue has
    no clock, so the cap is gone: the per-engine concurrency limits
    pace the providers, and the backlog drains in order.
    """

    def _busy_scan(self):
        """Create a scan that holds one PENDING job row.
    ...
    def test_a_scan_is_claimed_while_others_hold_pending_work(self):
        for _ in range(6):
            self._busy_scan()
