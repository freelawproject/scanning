...
document.addEventListener('DOMContentLoaded', function () {
    ...
    function createPlaceholders(numPages) {
        ...
        var entries = pageMap.length > 0 ? pageMap : [];
        if (entries.length === 0) {
    ...
    function createPdfPlaceholder(entry) {
        ...
        div.innerHTML =
            ...
            '    <button class="redetect-btn" title="Re-detect with another model">Re-detect</button>' +
        ...
        if (!viewOnly) {
            ...
            var redetectBtn = div.querySelector('.redetect-btn');
            ...
            (function (pageDiv, pNum, pdfIdx) {
                ...
                redetectBtn.addEventListener('click', function () { redetectPage(pageDiv, pNum, this); });
    ...
    // ── Scan for missing label ──
    window.scanForLabel = function() {
        var labels = ['STATE_ABBREVIATION', 'CASE_CAPTION', 'KEY_ICON', 'HEADNOTE',
                      'PAGE_NUMBER', 'PAGE_HEADER', 'DIVIDER', 'CASE_SEQUENCE',
                      'HEADNOTE_BRACKET', 'FOOTNOTES', 'EDITORIAL', 'CASE_METADATA'];
        var label = prompt('Label to scan for:\n' + labels.join(', '), 'STATE_ABBREVIATION');
        if (!label) return;

        var model = prompt('Model (small / medium / large):', 'large');
        if (!model) return;

        var range = prompt('Page range (e.g. 1-20, or blank for all):', '');
        var pageStart = 0, pageEnd = null;
        if (range) {
            var parts = range.split('-');
            pageStart = parseInt(parts[0]) - 1;
            pageEnd = parts.length > 1 ? parseInt(parts[1]) : pageStart + 1;
        }
        ...
        .then(function(r) { return r.json(); })
        .then(function(data) {
            btn.textContent = 'Scan for Label';
            btn.disabled = false;
            if (data.error) {
                alert('Error: ' + data.error);
            } else {
                ...
                if (data.added > 0) {
                    ...
                    .then(function() {
                        ...
                        refreshOverlays();
                        detectionsVisible = {};
                        document.querySelectorAll('.detection-box').forEach(function(el) { el.remove(); });
                        clearOverlaysByClass('redaction-overlay-box');
                        if (redactionsVisible) {
                            redactionsVisible = false;
                            toggleRedactions(); // re-fetch and show
                        }
                    });
                }
            }
    ...
    // ── Re-detect a page with another model ──
    // Model agreement colors
    var _modelColors = {
        3: {border: '#059669', bg: 'rgba(5,150,105,0.12)', label: '#059669'},   // all 3 agree — green
        2: {border: '#f59e0b', bg: 'rgba(245,158,11,0.10)', label: '#d97706'},  // 2 agree — amber
        1: {border: '#ef4444', bg: 'rgba(239,68,68,0.08)', label: '#dc2626'},   // only 1 — red
    };

    function _modelSummary(det) {
        if (!det.found_by || !det.found_by.length) return det.label;
        return det.found_by.map(function(f) {
            return f.model[0].toUpperCase() + ':' + f.confidence;
        }).join('  ');
    }
    ...
    function redetectPage(pageDiv, pageNum, btn) {
        ...
        .then(function(data) {
            ...
            var newDets = data.detections;
            if (!newDets.length) { alert('No new detections found'); return; }

            var wrapper = pageDiv.querySelector('.canvas-wrapper');
            var canvas = pageDiv.querySelector('.pdf-canvas');
            if (!wrapper || !canvas) return;

            var imgW = newDets[0].img_width || 1;
            var imgH = newDets[0].img_height || 1;
            var sx = canvas.offsetWidth / imgW;
            var sy = canvas.offsetHeight / imgH;

            // Clear previous preview boxes
            wrapper.querySelectorAll('.redetect-preview').forEach(function(el) { el.remove(); });

            newDets.forEach(function(d) {
                var mc = Math.min(d.model_count || 1, 3);
                var colors = _modelColors[mc];
                var box = document.createElement('div');
                box.className = 'redetect-preview';
                box.style.cssText = 'position:absolute;border:2px dashed ' + colors.border + ';background:' + colors.bg + ';z-index:12;cursor:pointer;';
                box.style.left = (d.bbox[0] * sx) + 'px';
                box.style.top = (d.bbox[1] * sy) + 'px';
                box.style.width = ((d.bbox[2] - d.bbox[0]) * sx) + 'px';
                box.style.height = ((d.bbox[3] - d.bbox[1]) * sy) + 'px';
                ...
                // Label badge
                var label = document.createElement('span');
                label.style.cssText = 'position:absolute;top:-16px;left:0;font-size:9px;padding:1px 3px;color:white;background:' + colors.label + ';border-radius:2px;white-space:nowrap;';
                label.textContent = d.label + ' (' + mc + '/3)';
                box.appendChild(label);

                // Hover tooltip with per-model breakdown
                var tooltip = document.createElement('div');
                tooltip.className = 'redetect-tooltip';
                tooltip.style.cssText = 'display:none;position:absolute;bottom:calc(100% + 4px);left:0;background:#1f2937;color:#f3f4f6;font-size:10px;padding:6px 8px;border-radius:4px;z-index:30;white-space:nowrap;pointer-events:none;line-height:1.5;';
                var tipLines = ['<b>' + d.label + '</b>'];
                ['small', 'medium', 'large'].forEach(function(m) {
                    var found = (d.found_by || []).find(function(f) { return f.model === m; });
                    tipLines.push(m + ': ' + (found ? '<span style="color:#34d399">' + found.confidence + '</span>' : '<span style="color:#6b7280">not found</span>'));
                });
                tooltip.innerHTML = tipLines.join('<br>');
                box.appendChild(tooltip);
                box.addEventListener('mouseenter', function() { tooltip.style.display = 'block'; });
                box.addEventListener('mouseleave', function() { tooltip.style.display = 'none'; });

                // Click to accept / unaccept
                (function(det, detBox) {
                    var _accepted = false;
                    detBox.addEventListener('click', function(e) {
                        ...
                        if (!_accepted) {
                            ...
                            .then(function(resp) {
                                _accepted = true;
                                detBox.style.borderColor = '#059669';
                                detBox.style.borderStyle = 'solid';
                                detBox.style.background = 'rgba(5,150,105,0.15)';
                                detBox.querySelector('span').style.background = '#059669';
                                detBox.querySelector('span').textContent = det.label + ' (accepted)';
                                allDetections = null;
                                refreshOverlays();
                                ...
                                if (_pLabels3.indexOf(det.label) >= 0) {
                                    ...
                                    .then(function(data2) {
                                        alert((resp.added ? 'Added ' + det.label + '. ' : '') + (data2.message || ''));
                                        window.location.reload();
                                    });
                                }
                            });
                            ...
                            fetch('/scans/' + documentId + '/save-redaction-rect/', {
                                ...
                                headers: {'X-CSRFToken': csrfToken, 'Content-Type': 'application/json'},
                                body: JSON.stringify({
                                    page_index: det.page_index, action: 'delete',
                                    original: {x0: det.bbox[0], y0: det.bbox[1], x1: det.bbox[2], y1: det.bbox[3]},
                                    type: det.label,
                                }),
                            }).then(function() {
                                _accepted = false;
                                var mc2 = Math.min(det.model_count || 1, 3);
                                var c2 = _modelColors[mc2];
                                detBox.style.borderColor = c2.border;
                                detBox.style.borderStyle = 'dashed';
                                detBox.style.background = c2.bg;
                                detBox.querySelector('span').style.background = c2.label;
                                detBox.querySelector('span').textContent = det.label + ' (' + mc2 + '/3)';
                                allDetections = null;
                                refreshOverlays();
                            });
                        }
            ...
            // Toolbar: accept-all / dismiss
            var toolbar = document.createElement('div');
            toolbar.className = 'redetect-preview';
            toolbar.style.cssText = 'position:absolute;top:4px;right:4px;z-index:25;display:flex;gap:4px;';
            ...
            dismissBtn.style.cssText = 'background:#6b7280;color:white;border:none;padding:4px 10px;border-radius:4px;cursor:pointer;font-size:12px;';
